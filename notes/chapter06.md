...
1. 角色扮演：在 CAMEL 最初的设计中，一个任务通常由两个智能体协作完成。这两个智能体被赋予了互补的、明确定义的“角色”。一个扮演“AI 用户” (AI User)，负责提出需求、下达指令和构思任务步骤；另一个则扮演“AI 助理” (AI Assistant)，负责根据指令执行具体操作和提供解决方案。

2. 引导性提示：“引导性提示”是在对话开始前，分别注入给两个智能体的一段精心设计的、结构化的初始指令（System Prompt）。这段指令就像是为智能体植入的“行动纲领”，它通常包含以下几个关键部分：

- **明确自身角色**：例如，“你是一位资深的股票交易员...”
- **告知协作者角色**：例如，“你正在与一位优秀的 Python 程序员合作...”
- **定义共同目标**：例如，“你们的共同目标是开发一个股票交易策略分析工具。”
- **设定行为约束和沟通协议**：这是最关键的一环。例如，指令会要求 AI 用户“一次只提出一个清晰、具体的步骤”，并要求AI助理“在完成上一步之前不要追问更多细节”，同时规定双方需在回复的末尾使用特定标志（如 <SOLUTION>）来标识任务的完成。

### 优势与局限

1. 优势
- 最大的优势在于CAMEL"轻架构、重提示"的设计哲学。这种自然涌现的协作行为，往往比硬编码的工作流更加灵活和高效
- 多模态能力
- 工具集成
- 模型适配
- 生态联动
...
- **图结构建模**：将智能体逻辑抽象为有向图（Directed Graph），而不是传统的线性链式调用。
- **状态驱动**：所有节点共享一个状态对象（TypedDict 或 Pydantic BaseModel），通过 reducer 函数更新。
- **灵活编排**：支持条件分支、循环、并行等复杂控制流。
- **模块化与复用**：每个节点只完成单一职责，可组合成更复杂的工作流

### 框架结构

- **State（状态）**：共享数据结构，定义 Schema 和更新逻辑。
- **Node（节点）**：执行最小单元（如调用大模型、数据库查询），输入状态，输出更新后的状态。
- **Edge（边）**：根据当前状态决定下一步执行的节点，实现动态路由。
- **StateGraph**：核心类，用于定义和编译整个工作流。
- **MessageGraph**：专门用于聊天型 Agent，状态是消息列表。

### 优点

- **动态路由**：根据输入或中间结果灵活选择执行路径。
- **状态持久化**：自动维护上下文，避免手动传递。
- **模块化设计**：节点可复用，降低重复代码。
- **可视化调试**：流程图直观展示逻辑路径，便于定位问题。
- **容错机制**：支持错误重试、回滚策略，提高健壮性。

### 局限性

- **性能瓶颈**：在极大规模或高并发场景下，状态传递和图调度可能成为瓶颈。
- **生态依赖**：LangGraph 基于 LangChain 扩展，生态依赖较强，独立性有限。